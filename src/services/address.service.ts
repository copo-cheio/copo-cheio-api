...
export class AddressService {
  ...
  async findOrCreate(
    ...
    if (!address) {
      let eventAddress = addressString.split(",");
      let addressName = eventAddress[0].trim();
      let addressAddress = eventAddress[1].trim();
      let addressPostal = eventAddress[3].trim();
      ...
      address = await this.addressRepository.create({
        ...
        long_label: Array.isArray(addressString) ? addressString.join(','): addressString,
