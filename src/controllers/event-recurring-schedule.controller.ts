...
export class EventRecurringScheduleController {
  ...
  @post('/events/{id}/recurring-schedule', {
    responses: {
      '200': {
        ...
        content: {'application/json': {schema: getModelSchemaRef(RecurringSchedule)}},
      },
    },
  })
  async create(
  ...
  async patch(
    ...
    @param.query.object('where', getWhereSchemaFor(RecurringSchedule)) where?: Where<RecurringSchedule>,
  ): Promise<Count> {
    return this.eventRepository.recurringSchedule(id).patch(recurringSchedule, where);
  }
