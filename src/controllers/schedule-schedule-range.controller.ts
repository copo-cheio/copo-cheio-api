...
export class ScheduleScheduleRangeController {
  ...
  @post('/schedules/{id}/schedule-ranges', {
    responses: {
      '200': {
        ...
        content: {'application/json': {schema: getModelSchemaRef(ScheduleRange)}},
      },
    },
  })
  async create(
  ...
  async patch(
    ...
    @param.query.object('where', getWhereSchemaFor(ScheduleRange)) where?: Where<ScheduleRange>,
  ): Promise<Count> {
    return this.scheduleRepository.scheduleRanges(id).patch(scheduleRange, where);
  }
