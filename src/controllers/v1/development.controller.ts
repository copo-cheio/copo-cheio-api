...
function decrypt(cipherText: any = '') {
  ...
  // console.log(++i);
  const key: any = CryptoJS.enc.Utf8.parse(secret);
  // console.log(++i);
  ...
  // console.log(++i);
  const res = decryptedFromText.toString(CryptoJS.enc.Utf8);
  // console.log(++i);
...
export class DevelopmentController {
  ...
  */
  ...
  async queryParams(@param.path.string('model') model: string): Promise<any> {
    return new Promise(res => {
      setTimeout(() => {
