...
export class ContactController {
  ...
  async updateAll(
    ...
    if (contacts?.phone && contacts.phone.indexOf('+') == -1)
      contacts.phone = '+351' + contacts.phone;
    if (contacts?.email && !isValidEmail(contacts.email)) delete contacts.email;
    if (contacts?.website && !isValidHttpUrl(contacts.website))
  ...
  async findById(
    ...
    @param.filter(Contacts, {exclude: 'where'})
    filter?: FilterExcludingWhere<Contacts>,
  ): Promise<Contacts> {
