...
export class TagController {
  ...
  parseSinglePayload(payload: any) {
    ...
    for (let model of [
      ...
      let record = model[0];
      let arg = model[1];
      for (let key of status[arg].keys) {
        if (record[key]) {
  ...
  async newTag(
    ...
    const translationRecord = await this.translationRepository.create(translation.payload)
    const tagRecord = await this.tagRepository.create({...tag.payload,translationId:translationRecord.id})
  ...
  async updateTag(
    ...
    if(translation.update) await this.translationRepository.updateById(translation.id,translation.payload);
