...
export class ProductProductIngredientController {
  ...
  @get('/products/{id}/product-ingredients', {
    responses: {
      '200': {
        ...
        content: {
          'application/json': {
            schema: {type: 'array', items: getModelSchemaRef(ProductIngredient)},
          },
        },
      },
    },
  ...
  @post('/products/{id}/product-ingredients', {
    responses: {
      '200': {
        ...
        content: {'application/json': {schema: getModelSchemaRef(ProductIngredient)}},
      },
    },
  })
  async create(
  ...
  async patch(
    ...
    @param.query.object('where', getWhereSchemaFor(ProductIngredient)) where?: Where<ProductIngredient>,
  ): Promise<Count> {
    return this.productRepository.ingredients(id).patch(productIngredient, where);
  }
