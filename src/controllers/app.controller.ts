...
import {EventRepository,FavoriteRepository,PlaceRepository,UserRepository} from "../repositories";
...
/* ********************************** */
  ...
  async findFavorites(
    ...
    const favorites = await this.userRepository.getFavorites(this.currentUser.id)
    if(favorites.events.length>0){
      favorites.events = await this.eventRepository.findAll({...EventsQuery,where:{id:{inq:favorites.events}}})
    }
    if(favorites.places.length>0){
      favorites.places = await this.placeRepository.findAll({...PlacesQuery,where:{id:{inq:favorites.places}}})
    }
    return favorites
    // const results: any = await this.eventRepository.findNearUpcoming(
    //   latitude,
