...
export class OrderRepository extends SoftCrudRepository<
  ...
  OrderRelations
> {
  public readonly cartItems: HasManyRepositoryFactory<
    CartItem,
    typeof Order.prototype.id
  >;
  ...
  constructor(
    ...
    this.qr = this.createHasOneRepositoryFactoryFor(
      'qr',
    ...
    this.cartItems = this.createHasManyRepositoryFactoryFor(
      'cartItems',
      cartItemRepositoryGetter,
    );
    this.registerInclusionResolver(
      'cartItems',
      this.cartItems.inclusionResolver,
    );
  }
}
